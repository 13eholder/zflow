...
// 中文停用词（分词后过滤，避免虚词污染路由向量）
const STOP_ZH = new Set([
  '的', '了', '和', '在', '是', '我', '你', '他', '她', '它', '这', '那',
  '个', '一个', '们', '吗', '吧', '呢', '一下', '与', '及', '对', '把', '被', '让',
  '给', '到', '从', '帮我', '我们', '怎么', '如何', '什么', '帮我定',
]);

// 中文没有空格分词，用 Intl.Segmenter 按词切分；英文仍走原有管道。
const zhSegmenter =
...
function tokenize(text) {
  ...
  let rest = lower;
  if (zhSegmenter) {
    // 中文段用分词器切词，非中文段留给英文管道
    rest = '';
    for (const part of lower.split(/([一-鿿]+)/)) {
      if (/[一-鿿]/.test(part)) {
        for (const seg of zhSegmenter.segment(part)) {
          if (seg.isWordLike && !STOP_ZH.has(seg.segment)) tokens.push(seg.segment);
        }
      } else {
        rest += part + ' ';
      }
