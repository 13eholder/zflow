...
- 每个 task 有显式的验收条件
- Task 和验收条件使用稳定编号；存在 Contract 时按 `planning-and-task-breakdown` 建立引用和验证映射
...
- [ ] TASK-017：[描述]
  - AC-01：[完成后什么必须为真]
  - 验证：[引用 Contract 验证条目；无 Contract 时填写命令或检查]
  - 文件：[哪些文件将被触碰]
```
...
- **架构决策先形成 ADR**——按 `documentation-and-adrs` 在相关实现前编写并接受 ADR；Spec 引用决定，描述其要求的行为。批准 Spec 不替代批准 ADR。
- **当范围变化时更新**——添加或取消的功能应反映在规格中。
- **偏离先批准再执行**——需求、接口或架构约束的偏离必须先形成已批准的上游变更；临时例外也要明确适用范围、期限和恢复条件，并在当前有效 Spec／接口中引用。随后重新核验受影响的 Task Contract；Stage 的差异记录不产生放宽要求的权限。原始目标通过版本引用保留，不能以保留旧 Spec 替代维护当前有效规范。
- **提交规格**——规格属于版本控制，与代码一起。
