...
**先写 ADR，再实现。** 影响实现的架构选择必须在相关编码开始前形成 ADR 并获接受；存在 Task Contract 时，必须先于其 `READY`。Plan 和 Spec 引用已接受的决定，不能代替 ADR 记录或批准架构选择。Stage 的 ADR 前置要求按 `stage` 执行，不能等到封版补写决策依据。
...
- 实现后可以补充实际后果和证据；改变决定本身仍须先接受替代 ADR，再调整 Spec／接口及受影响 Contract。
- 临时架构例外同样需要先批准，明确范围、期限、恢复条件和被覆盖的约束；由 Stage 引用其批准依据，不能由差异表自行授权。
